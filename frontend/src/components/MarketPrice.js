...
function MarketPrice() {
  ...
  useEffect(() => {
    const getItems = () => {
      axios
        .get('https://africanmarketplace2.herokuapp.com/api/procducts', {})
        .then(response => {
  ...
  useEffect(() => {
    const results = marketItems.filter(items => {
      ...
      return (
        itemData.includes(searchItem.toLowerCase()) &&
        locData.includes(searchLoc.toLowerCase()) &&
        catData.includes(searchCat.toLowerCase())
      )
    })
      ...
      <SiteContainer>
        <SideBar>
          <TextBox>
            ...
            <br />
            <p>
              Here you can check local prices for goods, and use the information
              to make informed decisions on where to post your goods.
            </p>
          </TextBox>
        </SideBar>
        <FormContainer>
